-- Update 407 Bonner to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%407%bonner%' 
       OR p.name ILIKE '%407%bonner%'
       OR p.address ILIKE '%bonner%'
       OR p.name ILIKE '%bonner%');
...
BEGIN
    -- Get the lease for 407 Bonner
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%407%bonner%' 
           OR p.name ILIKE '%407%bonner%'
           OR p.address ILIKE '%bonner%'
           OR p.name ILIKE '%bonner%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 407 Bonner', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 407 Bonner';
    END IF;
END $$;
