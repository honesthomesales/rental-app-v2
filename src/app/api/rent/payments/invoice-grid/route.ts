...
export async function GET(request: Request) {
  try {
    ...
    currentFriday.setDate(today.getDate() + daysToFriday)
      ...
      adjustedFriday.setDate(currentFriday.getDate() + (weekOffset * 7))
      ...
      endFriday.setDate(adjustedFriday.getDate() + (1 * 7))
    ...
    const fridayDates = generateFridayColumns(startDate, endDate)
    console.log('Generated Friday dates:', fridayDates)
    console.log('Friday count:', fridayDates?.length || 0)
    console.log('Start date:', startDate, 'End date:', endDate)
    
    // Convert to the format expected by frontend
    const fridays = fridayDates.map(date => {
      const friday = new Date(date + 'T00:00:00.000Z') // Ensure UTC
      const month = friday.toLocaleDateString('en-US', { month: 'short' }).toUpperCase()
      const day = friday.getDate()
      
      // Check if this Friday is the actual current Friday
      const today = new Date()
      const currentDay = today.getDay()
      const daysToFriday = (5 - currentDay + 7) % 7
      const currentFriday = new Date(today)
      currentFriday.setDate(today.getDate() + daysToFriday)
      const isCurrent = friday.toISOString().split('T')[0] === currentFriday.toISOString().split('T')[0]
    ...
    const rows = leases?.map(lease => {
      ...
      const allPeriods = generatePeriodsForLease(lease, fridayDates)
      ...
      const cells = fridays.map((headerFriday, index) => {
        const cellFridayUTC = toUTC(headerFriday.date)
    ...
    return NextResponse.json({
      ...
      fridays: fridays.map(f => ({ date: f.date, label: f.label, isCurrent: f.isCurrent })),
      fridayData: fridays.map(f => ({ date: f.date, label: f.label, isCurrent: f.isCurrent })),
      rows: rows.map(row => ({
        ...row,
        cells: row.cells.map(cell => ({
          ...cell,
          amount: cell.expectedAmount,
          paid: cell.amountPaid,
          balance: cell.expectedAmount - cell.amountPaid,
          lateFee: 0, // TODO: Calculate actual late fees
          status: cell.status === 'outside_lease' ? 'no_invoice' : 
                  cell.status === 'inactive' ? 'no_invoice' :
                  cell.status === 'owed' ? 'unpaid' :
                  cell.status === 'partial' ? 'partial' :
                  cell.status === 'paid' ? 'paid' : 'no_invoice',
          periodStart: cell.windowStart,
          periodEnd: cell.windowEnd,
          invoiceId: `invoice-${row.lease.id}-${cell.dueDate}`,
          isCurrent: cell.fridayUTC && new Date(cell.fridayUTC).toDateString() === new Date().toDateString()
        }))
      })),
      debug: {
        leaseCount: leases?.length || 0,
