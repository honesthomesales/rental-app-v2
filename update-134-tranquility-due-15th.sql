-- Update 302 Austin to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%302%austin%' 
       OR p.name ILIKE '%302%austin%'
       OR p.address ILIKE '%austin%'
       OR p.name ILIKE '%austin%');
...
BEGIN
    -- Get the lease for 302 Austin
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%302%austin%' 
           OR p.name ILIKE '%302%austin%'
           OR p.address ILIKE '%austin%'
           OR p.name ILIKE '%austin%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 302 Austin', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 302 Austin';
    END IF;
END $$;
