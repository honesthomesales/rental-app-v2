-- Update 3050 S Livingston to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%3050%s%livingston%' 
       OR p.name ILIKE '%3050%s%livingston%'
       OR p.address ILIKE '%livingston%'
       OR p.name ILIKE '%livingston%');
...
WHERE "RENT_invoices".lease_id = l.id
  AND (p.address ILIKE '%3050%s%livingston%' 
    ...
    BEGIN
    -- Get the lease for 3050 S Livingston
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%3050%s%livingston%' 
           OR p.name ILIKE '%3050%s%livingston%'
           OR p.address ILIKE '%livingston%'
           OR p.name ILIKE '%livingston%')
    LIMIT 1;
        ...
        RAISE NOTICE 'Processed % months for 3050 S Livingston', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 3050 S Livingston';
        END IF;
    END $$;
