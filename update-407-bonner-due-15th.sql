-- Update 504 Grubb to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%504%grubb%' 
       OR p.name ILIKE '%504%grubb%'
       OR p.address ILIKE '%grubb%'
       OR p.name ILIKE '%grubb%');
...
BEGIN
    -- Get the lease for 504 Grubb
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%504%grubb%' 
           OR p.name ILIKE '%504%grubb%'
           OR p.address ILIKE '%grubb%'
           OR p.name ILIKE '%grubb%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 504 Grubb', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 504 Grubb';
    END IF;
END $$;
