-- Update 109 Hosch to be due on the 30th for all new invoices
-- This will update the lease rent_due_day and create missing invoices

-- Step 1: Update the lease to have rent_due_day = 30
UPDATE "RENT_leases" 
SET rent_due_day = 30
FROM "RENT_properties" p
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%109%hosch%' 
       OR p.name ILIKE '%109%hosch%'
       OR p.address ILIKE '%hosch%'
       OR p.name ILIKE '%hosch%');

-- Step 2: Update existing invoices to have due_date on the 30th
UPDATE "RENT_invoices" 
SET due_date = (
  DATE_TRUNC('month', period_start) + INTERVAL '29 days'
)
FROM "RENT_leases" l
...
WHERE "RENT_invoices".lease_id = l.id
  AND (p.address ILIKE '%109%hosch%' 
    ...
    BEGIN
    -- Get the lease for 109 Hosch
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%109%hosch%' 
           OR p.name ILIKE '%109%hosch%'
           OR p.address ILIKE '%hosch%'
           OR p.name ILIKE '%hosch%')
    LIMIT 1;
        ...
        IF lease_record.id IS NOT NULL THEN
            ...
            invoice_due_date := (month_date + INTERVAL '29 days')::DATE; -- 30th of the month
        ...
        RAISE NOTICE 'Processed % months for 109 Hosch', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 109 Hosch';
        END IF;
    END $$;
