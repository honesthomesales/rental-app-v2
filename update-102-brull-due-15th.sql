-- Update 313 C Street to be due on the 25th for all new invoices
-- This will update the lease rent_due_day and create missing invoices

-- Step 1: Update the lease to have rent_due_day = 25
UPDATE "RENT_leases" 
SET rent_due_day = 25
FROM "RENT_properties" p
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%313%c%street%' 
       OR p.name ILIKE '%313%c%street%'
       OR p.address ILIKE '%c%street%'
       OR p.name ILIKE '%c%street%');

-- Step 2: Update existing invoices to have due_date on the 25th
UPDATE "RENT_invoices" 
SET due_date = (
  DATE_TRUNC('month', period_start) + INTERVAL '24 days'
)
FROM "RENT_leases" l
...
BEGIN
    -- Get the lease for 313 C Street
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%313%c%street%' 
           OR p.name ILIKE '%313%c%street%'
           OR p.address ILIKE '%c%street%'
           OR p.name ILIKE '%c%street%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        WHILE month_date <= '2026-12-31' LOOP
            ...
            invoice_due_date := (month_date + INTERVAL '24 days')::DATE; -- 25th of the month
        ...
        RAISE NOTICE 'Processed % months for 313 C Street', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 313 C Street';
    END IF;
END $$;
