-- Update 120 Bowers to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%120%bowers%' 
       OR p.name ILIKE '%120%bowers%'
       OR p.address ILIKE '%bowers%'
       OR p.name ILIKE '%bowers%');
...
BEGIN
    -- Get the lease for 120 Bowers
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%120%bowers%' 
           OR p.name ILIKE '%120%bowers%'
           OR p.address ILIKE '%bowers%'
           OR p.name ILIKE '%bowers%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 120 Bowers', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 120 Bowers';
    END IF;
END $$;
