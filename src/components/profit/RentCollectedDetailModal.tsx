...
async function updateInvoiceBalance(invoiceId: string) {
  const invoiceResponse = await fetchAuthenticated(`/api/invoices/${invoiceId}`)
  ...
  const paymentsResponse = await fetchAuthenticated(
    `/api/payments?invoiceId=${encodeURIComponent(invoiceId)}`,
  )
  const payments = await paymentsResponse.json()
  const linkedPayments = Array.isArray(payments)
    ? payments.filter((p: { invoice_id?: string }) => p.invoice_id === invoiceId)
    : []
  ...
  const amountTotal = parseFloat(String(invoice.amount_total))
  const balanceDue = amountTotal - totalPaid
  const newStatus = balanceDue <= 0 ? 'PAID' : totalPaid > 0 ? 'PARTIAL' : 'OPEN'

  const updateResponse = await fetchAuthenticated(`/api/invoices?id=${invoiceId}`, {
    method: 'PUT',
...
export function RentCollectedDetailModal({
  ...
  const handleUpdatePayment = async () => {
    ...
    try {
      const response = await fetchAuthenticated(
        `/api/payments?id=${editingPayment.id}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (editingPayment.invoiceId) {
        await updateInvoiceBalance(editingPayment.invoiceId)
      }
  ...
  const handleDeletePayment = async (row: RentCollectedDetailRow) => {
    if (
      !window.confirm(
        `Delete this payment of ${formatMoney(row.amount)} from ${row.paymentDateLabel}?`,
      )
    ) {
      return
    }
    ...
    try {
      const response = await fetchAuthenticated('/api/payments', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (row.invoiceId) {
        await updateInvoiceBalance(row.invoiceId)
      }
