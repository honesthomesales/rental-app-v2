...
/**
 * Read-only rent-change preview for existing invoices.
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const mode = (searchParams.get("mode") ||
      "all_unpaid_partial") as RentApplyMode;
    ...
    const { data: lease, error: leaseError } = await supabaseServer
      .from("RENT_leases")
      .select(
        "id, rent, rent_cadence, status, property_id, tenant_id, lease_start_date, lease_end_date, RENT_properties(name, address), RENT_tenants(full_name, first_name, last_name)",
      )
