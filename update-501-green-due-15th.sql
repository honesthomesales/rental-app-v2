-- Update Countryside to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%countryside%' 
       OR p.name ILIKE '%countryside%');
...
BEGIN
    -- Get the lease for Countryside
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%countryside%' 
           OR p.name ILIKE '%countryside%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for Countryside', month_count;
    ELSE
        RAISE NOTICE 'No lease found for Countryside';
    END IF;
END $$;
