-- Update 501 Green to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%501%green%' 
       OR p.name ILIKE '%501%green%'
       OR p.address ILIKE '%green%'
       OR p.name ILIKE '%green%');
...
BEGIN
    -- Get the lease for 501 Green
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%501%green%' 
           OR p.name ILIKE '%501%green%'
           OR p.address ILIKE '%green%'
           OR p.name ILIKE '%green%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 501 Green', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 501 Green';
    END IF;
END $$;
