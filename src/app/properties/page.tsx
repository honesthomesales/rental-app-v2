  ...
  const fetchProperties = async () => {
    try {
      ...
      const propertiesWithLease = data.map((property: Property) => {
        // Find lease where current date is between lease_start_date and lease_end_date AND status is 'occupied'
        const activeLease = leaseData.find((lease: any) => {
          if (lease.property_id !== property.id) return false
          if (lease.status !== 'occupied') return false // Only consider occupied leases
