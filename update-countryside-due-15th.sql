-- Update 37 Caroline to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%37%caroline%' 
       OR p.name ILIKE '%37%caroline%'
       OR p.address ILIKE '%caroline%'
       OR p.name ILIKE '%caroline%');
...
BEGIN
    -- Get the lease for 37 Caroline
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%37%caroline%' 
           OR p.name ILIKE '%37%caroline%'
           OR p.address ILIKE '%caroline%'
           OR p.name ILIKE '%caroline%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 37 Caroline', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 37 Caroline';
    END IF;
END $$;
