-- Update 52 Humility to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%52%humility%' 
       OR p.name ILIKE '%52%humility%'
       OR p.address ILIKE '%humility%'
       OR p.name ILIKE '%humility%');
...
BEGIN
    -- Get the lease for 52 Humility
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%52%humility%' 
           OR p.name ILIKE '%52%humility%'
           OR p.address ILIKE '%humility%'
           OR p.name ILIKE '%humility%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 52 Humility', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 52 Humility';
    END IF;
END $$;
