-- Update 351 High to be due on the 30th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%351%high%' 
       OR p.name ILIKE '%351%high%'
       OR p.address ILIKE '%high%'
       OR p.name ILIKE '%high%');
...
WHERE "RENT_invoices".lease_id = l.id
  AND (p.address ILIKE '%351%high%' 
    ...
    BEGIN
    -- Get the lease for 351 High
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%351%high%' 
           OR p.name ILIKE '%351%high%'
           OR p.address ILIKE '%high%'
           OR p.name ILIKE '%high%')
    LIMIT 1;
        ...
        RAISE NOTICE 'Processed % months for 351 High', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 351 High';
        END IF;
    END $$;
