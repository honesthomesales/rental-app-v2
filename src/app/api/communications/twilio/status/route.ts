...
export async function POST(request: Request) {
  try {
    ...
    const { error: communicationError } = await supabaseServer
      .from("RENT_communications")
      .update(patch)
      .eq("id", existing.id);
    if (communicationError) {
      throw new Error("Failed to update verified delivery status");
    }
    ...
    if (mapped === "failed") {
      approvalPatch.provider_error_code = params.ErrorCode || null;
      approvalPatch.provider_error_message =
        String(params.ErrorMessage || "Provider delivery failed").slice(0, 500);
    }
    const { error: approvalError } = await supabaseServer
      .from("RENT_communication_approvals")
      .update(approvalPatch)
      .eq("sent_communication_id", existing.id)
      .in("status", ["sent", "sending"]);
    if (approvalError) {
      throw new Error("Failed to update approval delivery status");
    }
