-- Update 102 Brull to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%102%brull%' 
       OR p.name ILIKE '%102%brull%'
       OR p.address ILIKE '%brull%'
       OR p.name ILIKE '%brull%');
...
BEGIN
    -- Get the lease for 102 Brull
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%102%brull%' 
           OR p.name ILIKE '%102%brull%'
           OR p.address ILIKE '%brull%'
           OR p.name ILIKE '%brull%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 102 Brull', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 102 Brull';
    END IF;
END $$;
