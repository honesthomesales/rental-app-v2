-- Update 131 N Linda to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%131%n%linda%' 
       OR p.name ILIKE '%131%n%linda%'
       OR p.address ILIKE '%linda%'
       OR p.name ILIKE '%linda%');
...
WHERE "RENT_invoices".lease_id = l.id
  AND (p.address ILIKE '%131%n%linda%' 
    ...
    BEGIN
    -- Get the lease for 131 N Linda
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%131%n%linda%' 
           OR p.name ILIKE '%131%n%linda%'
           OR p.address ILIKE '%linda%'
           OR p.name ILIKE '%linda%')
    LIMIT 1;
        ...
        RAISE NOTICE 'Processed % months for 131 N Linda', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 131 N Linda';
        END IF;
    END $$;
