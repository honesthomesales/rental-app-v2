-- Update 166 Tullyton to be due on the 15th for all new invoices
...
WHERE "RENT_leases".property_id = p.id
  AND (p.address ILIKE '%166%tullyton%' 
       OR p.name ILIKE '%166%tullyton%'
       OR p.address ILIKE '%tullyton%'
       OR p.name ILIKE '%tullyton%');
...
BEGIN
    -- Get the lease for 166 Tullyton
    SELECT l.*, p.address, p.name
    INTO lease_record
    FROM "RENT_leases" l
    JOIN "RENT_properties" p ON l.property_id = p.id
    WHERE (p.address ILIKE '%166%tullyton%' 
           OR p.name ILIKE '%166%tullyton%'
           OR p.address ILIKE '%tullyton%'
           OR p.name ILIKE '%tullyton%')
    LIMIT 1;
    ...
    IF lease_record.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Processed % months for 166 Tullyton', month_count;
    ELSE
        RAISE NOTICE 'No lease found for 166 Tullyton';
    END IF;
END $$;
