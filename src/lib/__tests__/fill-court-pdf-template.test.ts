...
import {
  fillNorthCarolinaSummaryEjectmentPdf as fillNorthCarolinaByCoordinates,
  fillSouthCarolinaEjectmentPdf as fillSouthCarolinaByCoordinates,
} from '../court-pdf-fillers'
...
describe('fill-court-pdf-template', () => {
  ...
  it('fills SC template by coordinates and preserves official pages', async () => {
    const template = readFileSync(join(process.cwd(), 'public/forms/SCCA732.pdf'))
    const filled = await fillSouthCarolinaByCoordinates(
      sampleSouthCarolinaEjectmentFillData(),
      template
    )
    const doc = await PDFDocument.load(filled)
  ...
  it('fills NC template by coordinates and preserves official pages', async () => {
    const template = readFileSync(join(process.cwd(), 'public/forms/NC_eviction.pdf'))
    const filled = await fillNorthCarolinaByCoordinates(
      sampleNorthCarolinaEjectmentFillData(),
      template
    )
    const doc = await PDFDocument.load(filled)
